Shader "Projector/CausticsDistortMobile" {
	Properties {
		[Header(Color)]
	 ...
	 Subshader {
		...
		Pass {
			...
			fixed4 frag (v2f i) : COLOR {
				...
				fixed4 noise = tex2D(_NoiseTex, i.uvNoise-frac(_SinTime.y*_DistortionSpeed/10));
				noise = noise + (_SinTime.x*_CausticSpeed/10);
